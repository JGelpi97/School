//Modified version of Graph.java that can take a weighted graph file and make a non weighted graph
...
public class Graph {
	...
	//Extra 'weighted' field telling the graph its recieving a file that is weighted
	//But it does not use the weights and creates a regular graph
	public Graph(In in, int weighted) 
	{
        this(in.readInt());
        int E = in.readInt();
        for (int i = 0; i < E; i++) {
            int v = in.readInt();
            int w = in.readInt();
			double weight = in.readDouble();	//do nothing with
            addEdge(v, w);
        }
    }
