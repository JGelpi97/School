...
{	
	...
	if (raster_x < WINDOW_WIDTH && raster_y < WINDOW_HEIGHT)
	{
		...
		//Get 3D space coords
		gluUnProject(raster_x, raster_y, 1.0, modelViewMatrix, projMatrix, vp, &farx, &fary, &farz);

	
		//Fixed cam position
		GLfloat p[4] = {0, 0, -10, 1};

		GLfloat d[4] = {farx - 0, fary - 0, farz + 10};
		...
		for(int i = 0; i < shapeList.size(); i++) 
		{
			...
			if (result[0] > 0)		//If there was a hit - (either 1 or 2 intersections)
			{
				...
				b = 1.0f;
				a = 1.0f;
				//Plot a pixel where we checked for a hit
